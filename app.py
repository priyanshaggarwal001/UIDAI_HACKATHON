...
st.plotly_chart(
    px.bar(
        pd.concat([early_df.head(30), early_df.tail(30)]),
        x="state",
        y="early_ratio",
        text_auto=".2f",
...
table_df = pd.concat([
    latest_df[latest_df["risk"] == "🔴 High Risk"].sort_values("prob_increase").head(10),
    latest_df[latest_df["risk"] == "🟡 Medium Risk"].sort_values("prob_increase").head(8),
    latest_df[latest_df["risk"] == "🟢 Low Risk"].sort_values("prob_increase").head(7)
])
...
final_cols += ["prob_increase", "risk", "action", "total_enrolments", "year", "month"]
...
fig.update_yaxes(visible=False, range=[1, 0], fixedrange=True, scaleanchor="x", scaleratio=1)
fig.add_layout_image(dict(source=india_map_img, xref="paper", yref="paper", x=0.4, y=1, sizex=1, sizey=1, layer="below"))
fig.update_layout(autosize=False, margin=dict(l=0, r=0, t=30, b=0))
...
for cid, label in cluster_labels.items():
    ...
    plt.scatter(subset[:, 0], subset[:, 1], label=label, alpha=0.75, edgecolors="white", linewidths=0.5)
